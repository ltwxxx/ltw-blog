...
def add_lazy_loading_to_images(html_content):
    """为HTML中的所有图片添加懒加载属性"""
    # 匹配 <img> 标签，如果还没有 loading 属性，则添加 loading="lazy"
    def add_lazy(match):
        img_tag = match.group(0)
        # 如果已经有 loading 属性，直接返回
        if 'loading=' in img_tag:
            return img_tag
        # 处理自闭合标签 <img ... /> 或普通标签 <img ... >
        if img_tag.endswith('/>'):
...
def generate_project_list_page():
    ...
    # 项目列表页在 project/index.html，链接需相对 project/ 目录
    for project in projects:
        project['url'] = f"{project.get('project_path', '')}/content.html"

    # 为项目列表页面调整图片路径（移除./前缀），并设置 cover 供模板使用
    for project in projects:
        if project.get('image') and not project['image'].startswith('http'):
            if project['image'].startswith('./'):
                project['image'] = project['image'][2:]  # 移除 ./
            # 列表页模板使用 project.cover，相对 project/index.html 的路径
            project['cover'] = f"{project.get('project_path', '')}/{project['image']}".lstrip('/')
        else:
            project['cover'] = project.get('image') or ''
...
def generate_projects_preview_html():
    ...
    # 为主页预览设置链接（与 url 一致，供 project_preview 模板的 project.link 使用）
